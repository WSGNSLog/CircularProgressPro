...
- (NSAttributedString *)formatProgressStringFromTimeInterval:(NSTimeInterval)interval {
    ...
    if (_status.length > 0) {
        ...
        attributedString = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n%@s",_status, progressString]];
        
        [attributedString addAttributes:@{
                                          NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue-thin" size:18],NSForegroundColorAttributeName:[UIColor lightGrayColor]}
                                  range:NSMakeRange(0, _status.length)];
        [attributedString addAttributes:@{
                                          NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue-thin" size:20]}
                                  range:NSMakeRange(progressString.length+_status.length+1, 1)];
        
        [attributedString addAttributes:@{
                                          NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:60]}
                                  range:NSMakeRange(_status.length+1, progressString.length)];
